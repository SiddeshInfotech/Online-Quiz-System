...
const StatCard = ({ icon: Icon, iconColor, value, label, delay = 0 }) => (
  <motion.div
    ...
    className="surface rounded-2xl p-5 border border-app shadow-sm flex flex-col items-center justify-center text-center hover:shadow-md hover:border-violet-500/30 transition-all cursor-default"
  >
...
const ResultStats = ({ result, isLoading }) => {
  if (isLoading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="animate-pulse surface-elev h-32 rounded-2xl"></div>
        ))}
      </div>
    );
  }
  ...
  const stats = [
    { icon: FileText, iconColor: "surface-elev text-app-2", value: result.total_questions ?? "--", label: "Questions" },
